...
<u><b>Обратите внимание:</b></u> таким образом можно "отредактировать" название только что добавленного раздела, которого еще нет в базе данных. В противном случае вы получите отказ
...
3. кнопка `...` ("горячая клавиша" `Shift+Ctrl+6`) для быстрого добавления услуги путем выбора из [списка услуг базового прайса](#47_mnu_price_list).
	...
	<u><b>Обратите внимание:</b></u> во всплывающем окне выбора услуги будет отображаться список медицинских услуг, которые "принадлежат" тому разделу базового прайса, в который мы эти услуги добавляем (в приведенном примере, это раздел "Цитология")
...
<u><b>Обратите внимание:</b></u> в списке прайсов у нас отображается пока только один пункт, зато стали доступны кнопки добавления(из уже имеющегося прайса)/редактирования/удаления текущего прайс листа. Также мы можем фильтровать список по разделам базового прайса
...
3. группа кнопок для добавления/редактирования/удаления разделов и медицинских услуг. 
	...
	<u><b>Обратите внимание:</b></u> В этом режиме создания прайса разделы базового прайса можно добавлять и удалять, но нельзя редактировать. Также кнопки удаления/восстановления медицинских услуг будут доступны соответственно состоянию элемента списка
...
<u><b>Обратите внимание:</b></u> если медицинская услуга была уже включена хотя бы в один прайс-лист, то вам будет отказано в ее удалении
...
1. ***Рецепт*** - кнопка позволяет вывести на печать рецепт выделенного в списке препарата
	...
	<u><b>Обратите внимание:</b></u> если вы не заполнили поле ввода "Рецептурная пропись" для препарата, то на печать будет выведен либо пустой бланк рецепта, либо тот текст, который указан в этом поле по умолчанию
...
<u><b>Обратите внимание:</b></u> что здесь вы можете только распечатать ранее введенные результаты. Если вам необходимо изменить их, то сделать это можно только, [отредактировав данные визита](#3841_mnu_patient_visit_labor_common) на эту дату.
...
6. Кнопки переноса медицинских услуг из списка доступных (расположен слева) в список назначаемых пациенту услуг (расположен справа). Переносить выбранные элементы слева направо и наоборот можно  
	...
	- путем перетаскивания мышью	
		...
		<u><b>Обратите внимание:</b></u> сразу несколько записей можно отметить, если держать зажатой кнопку `Ctrl` и помечать записи левой кнопкой мыши
...
<u><b>Обратите внимание:</b></u> на скрине выше (выделено рамкой) отображен контактный номер телефона врача. "Видимость" этого номера в отчете можно изменить, отметив галочку "Для служебного использования" при внесении данных доктора в [справочнике врача](#42_mnu_medical) (цифра `2` на скрине). 
...
<u><b>Обратите внимание:</b></u> при наличии хотя бы одной записи в списке становятся доступными кнопки `Редактировать` ("горячая клавиша" `Ctrl+E`) и ` Удалить` ("горячая клавиша" `Ctrl+D`). 
...
<u><b>Обратите внимание:</b></u> online-справка позволяет сразу открывать "нужный" раздел справки, например, из окна по нажатию кнопки `Помощь`. Локальная справка такой возможности лишена и "нужный" раздел вам придется открывать из оглавления.
...
Меню вызывается после нажатия правой кнопкой мыши (ПКМ) по общему списку пациентов

![](pict/071_mnu_db_01.png)

Пункты меню и субменю повторяют функционал действий, которые были описаны выше.
...
Меню вызывается после нажатия правой кнопкой мыши (ПКМ) по списку визитов пациента к врачам

![](pict/072_mnu_db_01.png)

Большинство пунктов меню и субменю повторяют функционал действий, которые были описаны выше.

Подробнее остановлюсь лишь на двух пунктах меню, функционал которых описывается в первый раз

***Перенести текущий визит*** <a name="721_mnu_db_popup_detail"></a> - переносит текущий визит к другому пациенту.  Применяется в случае, когда вы ошибочно оформили визит "не на того" пациента.

![](pict/072_mnu_db_02.png)

Вызов этого действия откроет окно

![](pict/072_mnu_db_03.png)

где можно отфильтровать обширный список с пациентами по ФИО и выбрать пациента, которому бы хотели присвоить "неправильно" оформленный визит. После нажатия кнопки `Выбрать` выбранный визит будет перенесен от текущего пациента к выбранному 

![](pict/072_mnu_db_04.gif)

***Перенести все визиты и удалить пациента*** - переносит **все** визиты текущего пациента к выбранному, а текущий пациент удаляетмся из базы данных. Применяется в основном "для борьбы" с дубликатами записей

![](pict/072_mnu_db_05.png)

Вызов этого действия откроет окно

![](pict/072_mnu_db_06.png)

где можно "поискать" дубликат, пометив чекбокс "Искать совпадение с текущим пациентом", и выбрав пациента, которому будут перенесены все визиты текущего

![](pict/072_mnu_db_07.gif)
...
Меню вызывается после нажатия правой кнопкой мыши (ПКМ) по иконке приложения в трее ("свернуть" приложение в трей можно, попытавшись нажать на "крестик" в верхнем правом углу окна приложения, или нажав "горячую клавишу" `Esc`)

![](pict/073_mnu_db_01.png)

В меню всего два пункта, позволяющие либо восстановить окно приложения, либо выйти из него

![](pict/073_mnu_db_02.gif)
